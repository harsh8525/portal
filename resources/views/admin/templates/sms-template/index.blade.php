                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $smsTemplateData->firstItem() ? $smsTemplateData->firstItem() :'0' }} to {{ $smsTemplateData->lastItem() ? $smsTemplateData->lastItem() :'0' }}
                          of total {!! $smsTemplateData->total() !!} entries (filtered from {{ $smsTemplateDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $smsTemplateData->firstItem() ? $smsTemplateData->firstItem() :'0' }} to {{ $smsTemplateData->lastItem() ? $smsTemplateData->lastItem() :'0' }}
                          of total {!! $smsTemplateData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('sms-template.index')}}">
                    ...
                    <div class="table-card">
                      ...
                      <div class="card-body table-radius table-responsive p-0">
                        <table class="table table-head-fixed text-nowrap">
                          ...
                          @forelse($smsTemplateData as $key=>$data)
                            <tr>
                              <th class="list-checkbox">
                              ...
                              <a href="{{ route('sms-template.edit',$data['id']) }}" title="Edit"> @forelse($data->smsCodeName as $sms_name)  
                                {{$sms_name['name']}} <br>
                                @endforeach</a>
                              </td>
                              <td class="table-action">
                                <span>
                                  <a href="{{ route('sms-template.edit',$data['id']) }}" title="Edit">
                       ...
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($smsTemplateData->hasPages()) { ?>
                          {!! $smsTemplateData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      <!-- /.card-body -->

            ...
            <div class="col-sm-12 d-flex breadcrumb-style">
              ...
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('cms-pages.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.cms-pages.delete',['delete_page_id'=>''])}}" data-bs-toggle="modal" data-bs-target="#exampleModal" title="Delete"
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $pageData->firstItem() ? $pageData->firstItem() :'0' }} to {{ $pageData->lastItem() ? $pageData->lastItem() :'0' }}
                          of total {!! $pageData->total() !!} entries (filtered from {{ $pageDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $pageData->firstItem() ? $pageData->firstItem() :'0' }} to {{ $pageData->lastItem() ? $pageData->lastItem() :'0' }}
                          of total {!! $pageData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <div class="collapse filter-collapse w-100" id="collapseExample">
                    <form autocomplete="off" id="filter" method="GET" action="{{route('cms-pages.index')}}">
                              ...
                              <div class="col-md-3 filter-form mb-3">
                                <div class="form-floating">
                                  <div class="form-item form-float-style">
                                    <input type="text" id="page_title" name="page_title" autocomplete="off" value="{{@$_GET['page_title']}}" class="is-valid">
                                    <label for="sort">Page Title</label>
                                  </div>
                                </div>
                              </div>
                  ...
                  <div class="col-md-12">
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            ...
                            <th class="list-checkbox">
                              ...
                              <th class="no-data-list"> <span>Sr. No. </span></th>
                              <th>Page Title <a href="{{Request::url().$queryStringConcat}}&order_by=page_title&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                  ...
                                  <th>Modified Date <a href="{{Request::url().$queryStringConcat}}&order_by=created_at&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                          ...
                          <tr class="">
                            <th class="list-checkbox">
                                <input class="form-check-input" name="check" type="checkbox" value="{{$page['id']}}"
                                  id="flexCheckDefault">
                              ...
                              <td>
                              @forelse($page->pageCodeName as $page_title)  
                                {{$page_title['page_title']}} <br>
                                @endforeach
                              </td>
                              <td>
                                {{ getDateTimeZone($page->updated_at) }}
                              </td>
                              <td class="table-action">
                                <span>
                                  <a href='{{ route("cms-pages.show",$page->id) }}' title="View">
                                ...
                                <span>
                                  <a href='{{route("cms-pages.edit",$page->id)}}' title="Edit">
                                  ...
                                  <span>
                                    <a data-url="{{route('admin.cms-pages.delete',['delete_page_id'=>$page['id']])}}" type="button" title="Delete"
                      ...
                      <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($pageData->hasPages()) { ?>
                          {!! $pageData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      <!-- /.card-body -->

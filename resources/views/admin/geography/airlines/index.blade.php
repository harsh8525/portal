...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/css/select2.min.css">
          ...
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active"><span>@lang('airline.airlines')</span></li>
              </ol>
               <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('airlines.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.airlines.delete',['delete_airline_id'=>''])}}" title="Delete" class="deleteAllConfirmation btn-hover-del">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    @if($airlineData)
                     <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $airlineData->firstItem() ? $airlineData->firstItem() :'0' }} to {{ $airlineData->lastItem() ? $airlineData->lastItem() :'0' }}
                          of total {!! $airlineData->total() !!} entries (filtered from {{ $airlineDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $airlineData->firstItem() ? $airlineData->firstItem() :'0' }} to {{ $airlineData->lastItem() ? $airlineData->lastItem() :'0' }}
                          of total {!! $airlineData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                    @endif
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('airlines.index')}}">
                            ...
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="airline_name" name="airline_name" autocomplete="off" value="{{@$_GET['airline_name']}}" class="is-valid">
                                  <label for="sort">Airline Name</label>
                                </div>
                              </div>
                            </div>
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="airline_code" name="airline_code" autocomplete="off" value="{{@$_GET['airline_code']}}" class="is-valid">
                                  <label for="sort">Airline Code</label>
                                </div>
                              </div>
                            </div>
                    ...
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th> <span>Airline Name <a href="{{Request::url().$queryStringConcat}}&order_by=airline_name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                              ...
                              <th>Airline Code <a href="{{Request::url().$queryStringConcat}}&order_by=airline_code&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                          ...
                          @forelse($airlineData as $key=>$data)
                          @php
                           $className = "";
                          @endphp
                          ...
                          @endif
                            <tr class="{{$className}}">
                              ...
                              <td>
                              @forelse($data->airlineCodeName as $airline_code)
                                {{$airline_code['airline_name']}} <br>
                                @endforeach
                              </td>
                              <td>
                                {{$data['airline_code']}}
                              </td><?php $code = $data['airline_code']?>
                              <td>
                              <img data-toggle="popover" src="{{$data['airline_logo'] ? $data['airline_logo'] : URL::asset('assets/images/airlineLogo/'.$code.'.png')}}" alt="" height=52 width=90>
                              </td>
                                 <td>
                              ...
                              @if($data['deleted_at'] == null)
                                <span>
                                  <a href="{{ route('airlines.edit',$data['id']) }}" title="Edit">
                                ...
                                @if($data['deleted_at'] != null)
                                <span>
                                    <a href="{{ route('admin.airlines.restore',['restore_airline_id'=>$data['id']]) }}" type="button" title="Restore"
                                ...
                                @endif
                                 <span>
                                    <a data-url="{{route('admin.airlines.delete',['delete_airline_id'=>$data['id']])}}" type="button" title="Delete"
                      ...
                      @if($airlineData)
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($airlineData->hasPages()) { ?>
                          {!! $airlineData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      @endif
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
            <form id="airline_import" class="validate" method="post" action="{{ route('import-airline') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                  <div class="form-item form-float-style">
                      <input type="file" id="file" name="file" autocomplete="off" ></textarea>
                ...
                <div class="modal-footer">
                  ...
                  <a href="{{url('/')}}/sample_file/airline_sample.xls" class="btn btn-warning">Download Sample File</a>
                </div>

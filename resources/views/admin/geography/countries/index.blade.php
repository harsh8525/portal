          ...
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active"><span>@lang('country.countries')</span></li>
              </ol>
               <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('countries.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.countries.delete',['delete_country_id'=>''])}}" title="Delete" class="deleteAllConfirmation btn-hover-del">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    @if($countryData)
                     <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $countryData->firstItem() ? $countryData->firstItem() :'0' }} to {{ $countryData->lastItem() ? $countryData->lastItem() :'0' }}
                          of total {!! $countryData->total() !!} entries (filtered from {{ $countryDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $countryData->firstItem() ? $countryData->firstItem() :'0' }} to {{ $countryData->lastItem() ? $countryData->lastItem() :'0' }}
                          of total {!! $countryData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                    @endif
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('countries.index')}}">
                        ...
                        <div class="col-md-10">
                          <div class="row">
                            ...
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="isd_code" name="isd_code" autocomplete="off" value="{{@$_GET['isd_code']}}" class="is-valid">
                                  <label for="sort">ISD Code</label>
                                </div>
                              </div>
                            </div>
                    ...
                    <div class="table-card">
                      ...
                      <div class="card-body table-radius table-responsive p-0">
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                           ...
                           <th>ISD Code<a href="{{Request::url().$queryStringConcat}}&order_by=isd_code&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                            ...
                            <th> <span>Max Mobile Number Length</span></th>
                            <th>Status</th>
                            <th>Action</th>
                            </tr>
                          </thead>
                          <tbody class="td-data-color">
                          @forelse($countryData as $key=>$data)
                          @php
                           $className = "";
                          @endphp
                          ...
                          @endif
                            <tr class="{{$className}}">
                              ...
                              <td>
                              @forelse($data->countryCode as $country_code)
                              ...
                              <td class="table-action">
                                @if($data['deleted_at'] == null)
                                <span>
                                  <a href="{{ route('countries.edit',$data['id']) }}" title="Edit">
                                ...
                                @if($data['deleted_at'] != null)
                                <span>
                                    <a href="{{route('admin.countries.restore',['restore_country_id'=>$data['id']])}}" type="button" title="Restore"
                                ...
                                @endif
                                <span>
                                  <a data-url="{{route('admin.countries.delete',['delete_country_id'=>$data['id']])}}" type="button" title="Delete"
                      ...
                      @if($countryData)
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($countryData->hasPages()) { ?>
                          {!! $countryData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      @endif
      ...
      <div class="modal fade validate" id="exampleModal1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          ...
          <div class="modal-content">
                <form id="country_import" class="validate" method="post" action="{{ route('import-country') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-footer">
                      ...
                      <a href="{{url('/')}}/sample_file/country_sample.xls" class="btn btn-warning">Download Sample File</a>
                    </div>

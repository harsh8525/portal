          ...
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active"><span>@lang('state.states')</span></li>
              </ol>
               <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('states.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.states.delete',['delete_state_id'=>''])}}" title="Delete" class="deleteAllConfirmation btn-hover-del">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    @if($stateData)
                     <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $stateData->firstItem() ? $stateData->firstItem() :'0' }} to {{ $stateData->lastItem() ? $stateData->lastItem() :'0' }}
                          of total {!! $stateData->total() !!} entries (filtered from {{ $stateDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $stateData->firstItem() ? $stateData->firstItem() :'0' }} to {{ $stateData->lastItem() ? $stateData->lastItem() :'0' }}
                          of total {!! $stateData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                    @endif
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('states.index')}}">
                            ...
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="state_name" name="state_name" autocomplete="off" value="{{@$_GET['state_name']}}" class="is-valid">
                                  <label for="sort">State</label>
                                </div>
                              </div>
                            </div>
                            <div class="col-md-3 filter-form mb-3">
                    ...
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th>State Name <a href="{{Request::url().$queryStringConcat}}&order_by=state_name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                          ...
                          @endif
                            ...
                            <tr class="{{$className}}">
                              ...
                              <td>
                              @forelse($data->stateName as $state_name)
                              ...
                              @if($data['deleted_at'] == null)
                                <span>
                                  <a href="{{ route('states.edit',$data['id']) }}" title="Edit">
                                ...
                                @if($data['deleted_at'] != null)
                                <span>
                                    <a href="{{route('admin.states.restore',['restore_state_id'=>$data['id']])}}" type="button" title="Restore"
                                ...
                                @endif
                                 <span>
                                    <a data-url="{{route('admin.states.delete',['delete_state_id'=>$data['id']])}}" type="button" title="Delete"
                      ...
                      @if($stateData)
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($stateData->hasPages()) { ?>
                          {!! $stateData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      @endif
      ...
      <div class="modal fade validate" id="exampleModal1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          ...
          <div class="modal-content">
                <form id="state_import" class="validate" method="post" action="{{ route('import-state') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-footer">
                      ...
                      <a href="{{url('/')}}/sample_file/state_sample.xls" class="btn btn-warning">Download Sample File</a>
                    </div>

      ...
      <div class="card pb-3 pt-3 px-3 w-100">
        <div class="discount">
          ...
          <div class="grid-list-head py-2">
            <h3 class="mt-2 mb-0">Log Report
            </h3>
            <div class="gridlist-icons">

              <div class="export grid-icon-top" title="PDF">
                <a href="{{ route('admin.log-report.pdf',['logData' => $logData,  'fromDate'=>$fromDate, 'toDate'=> $toDate]) }}" target="_blank" class="btn btn-success float-right plush-icon"><i class="far fa-file-pdf"></i></a>
                </a>
              </div>
              <div class="export grid-icon-top" title="Export">
                <a href="{{ route('admin.log-report.export',['logData' => $logData,  'fromDate'=>$fromDate, 'toDate'=> $toDate]) }}" class="btn btn-success float-right plush-icon"><i class="fas fa-file-export"></i></a>
                </a>
              </div>
            ...
            <div class="filter-collapse w-100" id="collapseExample">
              <form autocomplete="off" id="filter" method="GET" action="{{route('reports.logReport.log-report')}}">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3 filter-buttons">
                      ...
                      <div class="filter-btm-btn" title="Refresh">
                        <a href="{{route('reports.logReport.log-report')}}" class="refress-filter">
                  ...
                  <table class="table table-head-fixed text-nowrap">
                    ...
                    <tbody class="td-data-color vertical-scroll-effect">
                      ...
                      <tr>
                        ...
                        <td>
                        <?php
                          $userName = App\Models\Customer::where('id',$data['customer_id'])->value('first_name');
                          ?>
                          @if($data['customer_id'] != 'guest')
                        <a target="_blank" href="{{ route('customers.show',$data['customer_id']) }}" title="View">{{ $userName ? $userName : $data['customer_id']}}</a>
                        @else
                        {{ $data['customer_id'] }}
                        @endif
                        </td>

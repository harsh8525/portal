    ...
    <section class="content">
        <div class="card">
            <div class="text-center pt-3 top-part" style="margin-top: 2rem;">
                ...
                <p class="table-ordreport mb-0 pt-2 space-rem">AGENCY LIST REPORT</p>
                    ...
                    <table id="dataList1" class="table table-bordered table-striped" style="width: 100%;">
                        <thead>
                            <tr role="row" style="background-color: #256BB7; border: 1px solid #dee2e6; color: #fff; font-weight: 600;">
                                ...
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Agency Name</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Short Name</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Agency Type</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Email</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Phone Number</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Operator Full Name</th>
                                <th class="table-head-notify" tabindex="0" aria-controls="example2" rowspan="1" colspan="1">Status</th>

                            </tr>
                        </thead>
                        <tbody> <?php $i = 0; ?> @foreach($agencyData AS $key=>$data)

                            <tr class="table-data-con">
                                <td class="text-center">{{++$i}}</td>
                                <td class="text-center">{{$data->full_name}}</td>
                                <td class="text-center">{{ucwords($data->short_name)}}</td>
                                <td class="text-center">@forelse($data->getAgencyType as $user)
                                    {{$user['name']}} <br>
                                    @endforeach
                                </td>
                                <td class="text-center">{{$data->email}}</td>
                                <td class="text-center">{{$data->phone_no}}</td>
                                <td class="text-center">@forelse($data->getMasterUserAgencyReport as $user)
                                {{$user['name']}} <br>
                                @endforeach</td>
                                <td class="text-center">{{ucwords($data->status)}}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>

  ...
  <div class="container-fluid">
    <div class="row mb-4 mt-2">
      <div class="col-sm-12 d-flex breadcrumb-style">
        <h1 class="m-0">Featured Flights - Edit</h1>
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="card pb-4 pt-3 px-3 w-100">
        <form id="dataForm" action="{{route('feature-flight.update',$featureflightDetail['id'])}}" class="form row pt-3 mb-0 validate" enctype="multipart/form-data" method="post">
          @csrf
          @method('PUT')
          <input type="hidden" name="redirects_to" id="redirects_to" value="{{ URL::previous() }}">
          <input type="hidden" value="{{$featureflightDetail['id']}}" name="feature_flight_id" id="feature_flight_id">
              ...
              <div class="form-floating form-item mb-0">
                <div class="form-item form-float-style serach-rem mb-0">
                  <div class="select top-space-rem after-drp form-float-style ">
                    <select data-live-search="true" id="airline_code" name="airline_code" class="order-td-input selectpicker1 select-text height_drp is-valid select2">
                      @foreach($getAirlinelist as $getAirlinecode)
                      <option value="{{$getAirlinecode->airline_code}}" @if($getAirlinecode->airline_code == $featureflightDetail->airline_code)
                        {{'selected="selected"'}}
                        @endif>@foreach($getAirlinecode['airlineCodeName'] as $airline) {{$airline->airline_name}} @endforeach
                      </option>
                      @endforeach
                    </select>
                    <label class="select-label searchable-drp" style="font-size: 12px; font-weight: 400 !important;">Airline</label>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="form-item form-float-style serach-rem mb-0">
                  <div class="select top-space-rem after-drp form-float-style ">
                    <select data-live-search="true" id="from_airport_code" name="from_airport_code" class="order-td-input selectpicker1 select-text height_drp is-valid select2">
                      @foreach($getAirportlist as $getAirportcode)
                      <option value="{{$getAirportcode->iata_code}}" @if ($getAirportcode->iata_code==$featureflightDetail->from_airport_code)
                        {{'selected="selected"'}}
                        @endif>
                        @foreach($getAirportcode['airportName'] as $airport)
                        {{$airport->airport_name}}
                        @endforeach
                      </option>
                      @endforeach
                    </select>
                    <label class="select-label searchable-drp" style="font-size: 12px; font-weight: 400 !important;">From Airport</label>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="col-md-12 row mb-3">
            <div class="col-md-6">
              <div class="form-floating form-item mb-0">
                <div class="form-item form-float-style serach-rem mb-0">
                  <div class="select top-space-rem after-drp form-float-style ">
                    <select data-live-search="true" id="to_airport_code" name="to_airport_code" class="order-td-input selectpicker1 select-text height_drp is-valid select2">
                      @foreach($getAirportlist as $getAirportcode)
                      <option value="{{$getAirportcode->iata_code}}" @if ($getAirportcode->iata_code==$featureflightDetail->to_airport_code)
                        {{'selected="selected"'}}
                        @endif>
                        @foreach($getAirportcode['airportName'] as $airport)
                        {{$airport->airport_name}}
                    ...
                    <label class="select-label searchable-drp" style="font-size: 12px; font-weight: 400 !important;">To Airport</label>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="col-md-12 row mb-3">
            <div class="col-md-6">
              <div class="form-item form-float-style">
                <input name="price" type="number" id="price" autocomplete="off" value="{{ $featureflightDetail['price'] }}">
                ...
                <div class="form-item form-float-style serach-rem mb-0">
                  <div class="select top-space-rem after-drp form-float-style ">
                    <select data-live-search="true" id="status" name="status" class="order-td-input selectpicker select-text height_drp is-valid">
                      <option @if($featureflightDetail['status']=='1' ) selected="selected" @endif value="1">Active</option>
                      <option @if($featureflightDetail['status']=='0' ) selected="selected" @endif value="0">In-active</option>
                    </select>

            ...
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('suppliers.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('suppliers.delete',['supplier_id'=>''])}}" data-bs-toggle="modal" data-bs-target="#exampleModal" title="Delete"
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $supplierData->firstItem() ? $supplierData->firstItem() :'0' }} to {{ $supplierData->lastItem() ? $supplierData->lastItem() :'0' }}
                          of total {!! $supplierData->total() !!} entries (filtered from {{ $supplierDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $supplierData->firstItem() ? $supplierData->firstItem() :'0' }} to {{ $supplierData->lastItem() ? $supplierData->lastItem() :'0' }}
                          of total {!! $supplierData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('suppliers.index')}}">
                            ...
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="suppliers_name" name="suppliers_name" autocomplete="off" value="{{@$_GET['suppliers_name']}}" class="is-valid">
                    ...
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th>Code  <a href="{{Request::url().$queryStringConcat}}&order_by=code&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                              ...
                              <th>Name  <a href="{{Request::url().$queryStringConcat}}&order_by=name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                  ...
                                  <th>Service Type  <a href="{{Request::url().$queryStringConcat}}&order_by=core_service_type_id&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                          ...
                          @forelse($supplierData as $key=>$data)
                                <tr>
                                  ...
                                  <a href="{{ route('suppliers.show',$data['id']) }}" title="View">{{$data['name']}}</a>
                                  </td>
                                  <td>{{$data['service_name']}}</td>
                                  <td class="tablelist-img">
                                    <img data-toggle="popover" src="{{ $data['cover_image'] ?: URL::asset('assets/images/no-image.png')}}" alt="">
                                  </td>
                                  <td>
                                  <?xml version="1.0"  encoding="utf-8"?>
                                    ...
                                    <?php }else if($data['is_active'] == 2){ //delete ?>
                                    
                                    <?php } ?>
                                  </td>
                                  <td class="table-action">
                                  @if($data['code'] == 'AMADEUS')
                                  <span><a href="{{ route('amadeus-api.index') }}"title="amdeus">
                                      <i class="fa fa-building fa-lg"></i>
                                      </a></span>@endif

                                      @if($data['code'] == 'HOTEL_BEDS') <span><a href="{{ route('hotelbeds-api.index') }}"title="hotel_beds">
                                      <i class="fa fa-building fa-lg"></i>
                                      <!-- <i class="fa-solid fa-hotel fa-lg"></i> -->
                                      </a></span>@endif
                                  <span>
                                      <a href="{{ route('suppliers.show',$data['id']) }}" title="View">
                                    ...
                                    <span>
                                      <a href="{{ route('suppliers.edit',$data['id']) }}" title="Edit">
                                        <?xml version="1.0"?><svg fill="#198754" viewBox="0 0 24 24" width="20" height="20">
                                          <path
                                            d="M 19.171875 2 C 18.448125 2 17.724375 2.275625 17.171875 2.828125 L 16 4 L 20 8 L 21.171875 6.828125 C 22.275875 5.724125 22.275875 3.933125 21.171875 2.828125 C 20.619375 2.275625 19.895625 2 19.171875 2 z M 14.5 5.5 L 3 17 L 3 21 L 7 21 L 18.5 9.5 L 14.5 5.5 z" />
                                        </svg></a></span>
                                    <span>
                                      <a data-url="{{route('suppliers.delete',['supplier_id'=>$data['id']])}}" data-bs-toggle="modal" data-bs-target="#exampleModal" type="button" title="Delete"
                      ...
                      <nav class="pagination-grid" aria-label="Page navigation example">
                        <ul class="pagination">
                            <?php 
                            if($supplierData){
                            if($supplierData->hasPages()) { ?>
                          {!!$supplierData->appends(Request::except('page'))->render() !!}
                            <?php  }} ?>
                        </ul>
                      </nav>
                      <!-- /.card-body -->

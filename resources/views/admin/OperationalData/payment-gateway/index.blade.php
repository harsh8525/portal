            ...
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('payment-gateway.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('payment-gateway.delete',['payment_id'=> ''])}}" title="Delete" class="deleteAllConfirmation btn-hover-del">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  <h3 class="mt-2 mb-0">Payment Gateway List
                  </h3>
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $paymentData->firstItem() ? $paymentData->firstItem() :'0' }} to {{ $paymentData->lastItem() ? $paymentData->lastItem() :'0' }}
                          of total {!! $paymentData->total() !!} entries (filtered from {{ $paymentCountData }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $paymentData->firstItem() ? $paymentData->firstItem() :'0' }} to {{ $paymentData->lastItem() ? $paymentData->lastItem() :'0' }}
                          of total {!! $paymentData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('payment-gateway.index')}}">
                      ...
                      <div class="card-body table-radius table-responsive p-0">
                        <table class="table table-head-fixed text-nowrap">
                          ...
                          <tbody class="td-data-color">
                           ...
                           @forelse($paymentData as $key=>$data)
                               <?php 
                                $className = "";
                                if($data->is_active == '2' ){
                            ...
                            <tr class="{{$className}}">
                              ...
                              <td class="table-action">
                                 <span>
                                  <a href="#" title="Credentials"><i class="fa fa-key fa-lg"></i>
                                  </a></span>
                                <span>
                                  <a href="{{ route('payment-gateway.show',$data['id']) }}" title="View">
                              ...
                              @if($data['is_active'] != 2)
                                <span>
                                  <a href="{{ route('payment-gateway.edit',$data['id']) }}" title="Edit">
                                ...
                                <span>
                              @endif
                                  <a data-url="{{route('payment-gateway.delete',['payment_id'=>$data['id']])}}" type="button" title="Delete"
                       ...
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php
                            if($paymentData->hasPages()) { ?>
                          {!! $paymentData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      <!-- /.card-body -->

          ...
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">@lang('rolePermission.moduleHeading')</li>
              </ol>
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('role-permission.create') }}" title="Add">
                    ...
                    @lang('rolePermission.add')
                  </a>
                </div>
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.role-permission.delete',['role_id'=>''])}}" data-bs-toggle="modal" class ="deleteAllConfirmation" data-bs-target="#exampleModal" type="button" title="Delete">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  <h3 class="mt-2 mb-0">Role & Permissions List</h3>
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $rolePermissionData->firstItem() ? $rolePermissionData->firstItem() :'0' }} to {{ $rolePermissionData->lastItem() ? $rolePermissionData->lastItem() :'0' }}
                          of total {!! $rolePermissionData->total() !!} entries (filtered from {{ $rolePermissionCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $rolePermissionData->firstItem() ? $rolePermissionData->firstItem() :'0' }} to {{ $rolePermissionData->lastItem() ? $rolePermissionData->lastItem() :'0' }}
                          of total {!! $rolePermissionData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('role-permission.index')}}">
                          ...
                          <div class="row">
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" id="role_name" autocomplete="off"  name="role_name"  value="{{@$_GET['role_name']}}" class="is-valid">
                  ...
                  <div class="col-12">
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th>Role Name<a href="{{Request::url().$queryStringConcat}}&order_by=name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                              ...
                              <th class="table-heading">@lang('rolePermission.roleDescription')</th>
                               <th class="table-heading-width">@lang('rolePermission.role_type')</th>
                              <th class="no-data-list">@lang('rolePermission.modifiedDate') <a href="{{Request::url().$queryStringConcat}}&order_by=updated_at&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                              ...
                              <th>@lang('rolePermission.status')</th>
                              <th>@lang('rolePermission.action')</th>
                            </tr>
                          </thead>
                          <tbody class="td-data-color">
                            @foreach($rolePermissionData as $key=>$data)
                            <?php
                              $className = "";
                              if($data['status'] == 2){
                            ...
                            <tr  class="{{$className}}">
                              ...
                              <span>
                                  <a href="{{ route('role-permission.show',$data['id']) }}" title="View">
                                ...
                                @if($data->status != '2')
                                <span>
                                  <a href="{{ route('role-permission.edit',$data['id']) }}" title="Edit">
                                  ...
                                  <a   data-url="{{route('admin.role-permission.delete',['role_id'=>$data['id']])}}" data-bs-toggle="modal" class ="deleteConfirmation" data-bs-target="#exampleModal" type="button" title="Delete"><svg
                                      fill="#ee3137" viewBox="0 0 24 24" disabled width="20" height="20">
                                      <path
                                        d="M 10 2 L 9 3 L 5 3 C 4.4 3 4 3.4 4 4 C 4 4.6 4.4 5 5 5 L 7 5 L 17 5 L 19 5 C 19.6 5 20 4.6 20 4 C 20 3.4 19.6 3 19 3 L 15 3 L 14 2 L 10 2 z M 5 7 L 5 20 C 5 21.1 5.9 22 7 22 L 17 22 C 18.1 22 19 21.1 19 20 L 19 7 L 5 7 z M 9 9 C 9.6 9 10 9.4 10 10 L 10 19 C 10 19.6 9.6 20 9 20 C 8.4 20 8 19.6 8 19 L 8 10 C 8 9.4 8.4 9 9 9 z M 15 9 C 15.6 9 16 9.4 16 10 L 16 19 C 16 19.6 15.6 20 15 20 C 14.4 20 14 19.6 14 19 L 14 10 C 14 9.4 14.4 9 15 9 z" />
                                    </svg></a>
                                </span>
                      ...
                      <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($rolePermissionData->hasPages()) { ?>
                          {!! $rolePermissionData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      <!-- /.card-body -->

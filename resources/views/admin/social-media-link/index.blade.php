          ...
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              ...
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active"><span>{{$header['title']}}</span></li>
              </ol>
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  <h3>social Media Links List</h3>
                  <div class="gridlist-icons">
                    @if($socialMediaLinkData)
                     <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $socialMediaLinkData->firstItem() ? $socialMediaLinkData->firstItem() :'0' }} to {{ $socialMediaLinkData->lastItem() ? $socialMediaLinkData->lastItem() :'0' }}
                          of total {!! $socialMediaLinkData->total() !!} entries (filtered from {{ $socialMediaLinkDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $socialMediaLinkData->firstItem() ? $socialMediaLinkData->firstItem() :'0' }} to {{ $socialMediaLinkData->lastItem() ? $socialMediaLinkData->lastItem() :'0' }}
                          of total {!! $socialMediaLinkData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                    @endif
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('social-media-link.index')}}">
                      ...
                      <div class="row">
                            ...
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="name" name="name" autocomplete="off" value="{{@$_GET['airline_name']}}" class="is-valid">
                        ...
                        <div class="col-md-2 filter-buttons">
                          ...
                          <div class="refress-filter filter-btm-btn" title="Refresh">
                              <a href="{{route('social-media-link.index')}}">
                    ...
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th> <span>Name <a href="{{Request::url().$queryStringConcat}}&order_by=name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                          ...
                          @forelse($socialMediaLinkData as $key=>$data)
                            <tr class="">
                              <th class="list-checkbox">
                              ...
                              <td class="table-action">
                                <span>
                                  <a href="{{ route('social-media-link.edit',$data['id']) }}" title="Edit">
                            ...
                            @endforeach
                            @if($socialMediaLinkData)
                              @if($socialMediaLinkData->isEmpty())
                            <tr>
                      ...
                      @if($socialMediaLinkData)
                       <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($socialMediaLinkData->hasPages()) { ?>
                          {!! $socialMediaLinkData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      @endif

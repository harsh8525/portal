            ...
            <div class="col-sm-12 d-flex breadcrumb-style">
              ...
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('home-banner.create') }}" title="Add">
                ...
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.home-banner.delete',['banner_id'=>''])}}" data-bs-toggle="modal" data-bs-target="#exampleModal" title="Delete"
                ...
                <div class="grid-list-head py-2">
                  ...
                  <div class="gridlist-icons">
                    ...
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $bannerData->firstItem() ? $bannerData->firstItem() :'0' }} to {{ $bannerData->lastItem() ? $bannerData->lastItem() :'0' }}
                          of total {!! $bannerData->total() !!} entries (filtered from {{ $bannerDataCount }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $bannerData->firstItem() ? $bannerData->firstItem() :'0' }} to {{ $bannerData->lastItem() ? $bannerData->lastItem() :'0' }}
                          of total {!! $bannerData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                    @endif
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('home-banner.index')}}">
                        ...
                        <div class="col-md-10">
                          <div class="row">
                            <div class="col-md-3 filter-form mb-3">
                              <div class="form-floating">
                                <div class="form-item form-float-style">
                                  <input type="text" id="banner_name" name="banner_name" autocomplete="off" value="{{@$_GET['banner_name']}}" class="is-valid">
                                  <label for="sort">Banner Name</label>
                                </div>
                              </div>
                            </div>
                            ...
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" class="datepicker is-valid" name="from_date" id="from_date" value="{{ @$_GET['from_date'] }}" placeholder="DD/MM/YYYY" autocomplete="off">
                                <label for="datepicker">From Date</label>
                              </div>
                            </div>
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" class="datepicker is-valid" name="to_date" id="to_date" value="{{ @$_GET['to_date'] }}" placeholder="DD/MM/YYYY" autocomplete="off">
                              ...
                              <div class="form-floating form-item mb-0">
                                <div class="form-item form-float-style serach-rem mb-0">
                                  <div class="select top-space-rem after-drp form-float-style ">
                                    <select data-live-search="true" name="panel" class="order-td-input selectpicker select-text height_drp is-valid">
                                      <option value="" selected>Select Panel</option>
                                      <option value="b2c" @if($appliedFilter['panel'] == 'b2c') selected="selected" @endif >B2C</option>
                                      <option value="b2b" @if($appliedFilter['panel'] == 'b2b') selected="selected"  @endif>B2B</option>
                                      <option value="supplier" @if($appliedFilter['panel'] == 'supplier') selected="selected"  @endif>Supplier</option>
                                    </select>
                  ...
                  <div class="col-12">
                    <div class="table-card">
                        ...
                        <table class="table table-head-fixed text-nowrap">
                          <thead class="td-data-color">
                            <tr>
                              ...
                              <th>Banner Name <a href="{{Request::url().$queryStringConcat}}&order_by=name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                  ...
                                  <th>Image</th>
                              <th class="no-data-list">From Date<a href="{{Request::url().$queryStringConcat}}&order_by=from_date&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12"
                                    fill="#ffffff" shape-rendering="geometricPrecision"
                                    text-rendering="geometricPrecision" image-rendering="optimizeQuality"
                                    fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                                      d="M295.27 211.54H26.71c-6.23-.02-12.48-2.18-17.54-6.58-11.12-9.69-12.29-26.57-2.61-37.69L144.3 9.16c.95-1.07 1.99-2.1 3.13-3.03 11.36-9.4 28.19-7.81 37.58 3.55l129.97 157.07a26.65 26.65 0 0 1 7.02 18.06c0 14.76-11.97 26.73-26.73 26.73zM26.71 299.68l268.56-.01c14.76 0 26.73 11.97 26.73 26.73 0 6.96-2.66 13.3-7.02 18.06L185.01 501.53c-9.39 11.36-26.22 12.95-37.58 3.55-1.14-.93-2.18-1.96-3.13-3.03L6.56 343.94c-9.68-11.12-8.51-28 2.61-37.69 5.06-4.4 11.31-6.56 17.54-6.57z" />
                                  </svg></a></th>
                              <th class="no-data-list">To Date <a href="{{Request::url().$queryStringConcat}}&order_by=to_date&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12"
                                    fill="#ffffff" shape-rendering="geometricPrecision"
                                    text-rendering="geometricPrecision" image-rendering="optimizeQuality"
                                    fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                              ...
                              <th class="no-data-list">Sort Order <a href="{{Request::url().$queryStringConcat}}&order_by=sort_order&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                    shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                    image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                    viewBox="0 0 322 511.21">
                                    <path fill-rule="nonzero"
                              ...
                              <th class="no-data-list">Panel </th>
                              <th>Modified Date </th>
                              <th>Status</th>
                              <th>Action</th>
                            </tr>
                          ...
                          @forelse($bannerData as $key=>$data)
                            <tr>
                              <th class="list-checkbox">
                              ...
                              <td class="table_wrap_data">
                                 <a href="{{ route('home-banner.show',$data['id']) }}" title="View">@forelse($data->homeBannerCode as $banner_code)
                                {{$banner_code['banner_title']}} <br>
                                @endforeach</a>
                              </td>
                              <td class="tablelist-img-banner">
                                <img data-toggle="popover" src="{{$data['banner_image'] ?: URL::asset('assets/images/no-image.png')}}" alt=""> 
                              </td>
                              <td>
                              ...
                              <span>
                                  <a href="{{ route('home-banner.show',$data['id']) }}" title="View">
                                ...
                                <span>
                                  <a href="{{ route('home-banner.edit',$data['id']) }}" title="Edit">
                                    <?xml version="1.0"?><svg fill="#198754" viewBox="0 0 24 24" width="20" height="20">
                                      <path
                                        d="M 19.171875 2 C 18.448125 2 17.724375 2.275625 17.171875 2.828125 L 16 4 L 20 8 L 21.171875 6.828125 C 22.275875 5.724125 22.275875 3.933125 21.171875 2.828125 C 20.619375 2.275625 19.895625 2 19.171875 2 z M 14.5 5.5 L 3 17 L 3 21 L 7 21 L 18.5 9.5 L 14.5 5.5 z" />
                                    </svg></a></span>
                                <span>
                                  <a data-url="{{route('admin.home-banner.delete',['banner_id'=>$data['id']])}}" data-bs-toggle="modal" data-bs-target="#exampleModal" type="button" title="Delete"
                      ...
                      <nav class="pagination-grid" aria-label="Page  navigation example">
                        <ul class="pagination">
                            <?php if($bannerData->hasPages()) { ?>
                          {!! $bannerData->appends(Request::except('page'))->render() !!}
                            <?php } ?>
                        </ul>
                      </nav>
                      @endif

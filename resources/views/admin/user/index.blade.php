        ...
        <div class="container-fluid">
          <div class="row mb-4 mt-2">
            <div class="col-sm-12 align-items-center d-flex breadcrumb-style">
              <h1 class="m-0">@lang('adminUser.moduleHeading')</h1>
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">@lang('adminUser.dashboard')</a></li>
                <li class="breadcrumb-item active">@lang('adminUser.moduleHeading')</li>
              </ol>
              <div class="breadcrumb-btn">
                <div class="add-breadcrumb">
                  <a href="{{ route('user.create') }}" title="Add">
                    ...
                    @lang('adminUser.addNew')
                  </a>
                </div>
                <div class="delete-breadcrumb">
                  <a data-url="{{route('admin.user.delete',['admin_user_id'=>''])}}" title="Delete" class="deleteAllConfirmation btn-hover-del">
            ...
            <div class="card pb-3 pt-3 px-3 w-100">
              <div class="discount">
                ...
                <div class="grid-list-head py-2">
                  <h3 class="mt-2 mb-0">@lang('adminUser.adminUserList')
                  </h3>
                  <div class="gridlist-icons">
                    <?php if(isset($_GET['per_page'])  && $_GET['per_page'] !== ""){
                      ?>
                    <div>
                      <h6>Showing {{ $AdminUserData->firstItem() ? $AdminUserData->firstItem() :'0' }} to {{ $AdminUserData->lastItem() ? $AdminUserData->lastItem() :'0' }}
                          of total {!! $AdminUserData->total() !!} entries (filtered from {{ $AdminUserCountData }} total entries) </h6>
                    </div>
                    <?php } else {?> 
                    <div>
                       <h6>Showing {{ $AdminUserData->firstItem() ? $AdminUserData->firstItem() :'0' }} to {{ $AdminUserData->lastItem() ? $AdminUserData->lastItem() :'0' }}
                          of total {!! $AdminUserData->total() !!} entries</h6>
                    </div>
                    <?php } ?>
                  ...
                  <form autocomplete="off" id="filter" method="GET" action="{{route('user.index')}}">
                          ...
                          <div class="row">
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" id="name" autocomplete="off" name="name" value="{{@$_GET['name']}}" class="is-valid">
                                <label for="name">Full Name</label>
                              </div>
                            </div>
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" id="mobile" name="mobile" autocomplete="off" value="{{@$_GET['mobile']}}" class="is-valid">
                                <label for="mobile">Mobile Number</label>
                              </div>
                            </div>
                            <div class="col-md-3 filter-form">
                              <div class="form-item form-float-style">
                                <input type="text" id="email" name="email" autocomplete="off" value="{{@$_GET['email']}}" class="is-valid">
                             ...
                             <div class="col-md-3 filter-form">
                               <div class="form-item form-float-style">
                                    <div class="select top-space-rem after-drp form-float-style ">
                                       <select data-live-search="true" id="agancy_name" name="agancy_name" class="order-td-input selectpicker select-text height_drp is-valid">
                                        <option value="">Select Agency Name</option>
                                       @foreach($getAgencyName  as $getName) 
                                        <option value="{{ $getName->full_name }}"  @if($appliedFilter['agancy_name'] == $getName['full_name']) selected="selected" @endif>{{ $getName->full_name }} </option>
                                       @endforeach
                                      </select>
                                      <label class="select-label searchable-drp" style="font-size: 12px; font-weight: 400 !important;">Select Agency Name</label>
                                    </div>
                                  </div>
                                </div>
                            <div class="col-md-3 filter-form">
                              <div class="form-floating form-item mb-3">
                                <div class="form-item form-float-style serach-rem mb-3">
                                  <div class="select top-space-rem after-drp form-float-style ">
                                    <select data-live-search="true" name="user_role" class="order-td-input selectpicker select-text height_drp is-valid">
                                        <option value="" selected disabled>Select Role</option>
                                        @foreach(App\Models\Role::select('id','name','code')->get() as $value)
                                          <option   value="{{$value->code}}" @if(@$appliedFilter['user_role'] == $value->code) selected="selected" @endif>{{ucwords($value->name)}}</option>
                                        @endforeach
                                    </select>
                                    <label class="select-label searchable-drp" style="font-size: 12px; font-weight: 400 !important;">Role</label>
                                  </div>                        
                                </div>
                              </div>
                            </div>
              ...
              <div class="col-12">
                <div class="table-card">
                    ...
                    <table class="table table-head-fixed text-nowrap">
                      <thead class="td-data-color">
                        <tr>
                          ...
                          <th class="no-data-list"> <span>@lang('adminUser.srNo') </span></th>
                          <th>Agency Name <a href="{{Request::url().$queryStringConcat}}&order_by=agancy_name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                          ...
                          <th>@lang('adminUser.fullName') <a href="{{Request::url().$queryStringConcat}}&order_by=name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                              ...
                              <th>@lang('adminUser.image')</th>
                          <th>@lang('adminUser.mobileNumber') <a href="{{Request::url().$queryStringConcat}}&order_by=mobile&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                          ...
                          <th>@lang('adminUser.emailAddress') <a href="{{Request::url().$queryStringConcat}}&order_by=email&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                          ...
                          <th>@lang('adminUser.userRole') <a href="{{Request::url().$queryStringConcat}}&order_by=role_code&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                                ...
                                <th>App Name<a href="{{Request::url().$queryStringConcat}}&order_by=app_name&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                          ...
                          <th>@lang('adminUser.modifiedDate') <a href="{{Request::url().$queryStringConcat}}&order_by=updated_at&sorting={{($appliedFilter['sorting']=='desc') ? 'asc' : 'desc'}}"><svg width="12" height="12" fill="#ffffff"
                                shape-rendering="geometricPrecision" text-rendering="geometricPrecision"
                                image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd"
                                viewBox="0 0 322 511.21">
                                <path fill-rule="nonzero"
                      ...
                      <tbody class="td-data-color">
                        @foreach($AdminUserData as $key => $data)
                        <?php 
                        $className = "";
                        if($data['status'] == 2){
                        ...
                        <tr class="{{$className}}">
                          ...
                          <td class="no-data-list">
                            ...
                            $value = App\Models\Setting::where('config_key', 'general|basic|siteName')->get('value')[0]['value'];
                              ...
                              else if($data['agency_id'] > 0)
                              {
                                  echo  $data['agancy_name'];
                              } 
                            ?>
                          </td>
                          <td>
                          ...
                          <td class="tablelist-img">
                            <img data-toggle="popover" src="{{$data['profile_image'] ?: URL::asset('assets/images/no-image.png')}}" alt="">
                          </td>
                          <td class="no-data-list">
                            {{ $data['isd'] ?? "-" }} {{ $data['mobile'] ?? "-" }}
                          </td>
                          <td style="text-transform:lowercase">
                            {{ $data['email'] ?? "-" }}
                          </td>
                          <td>
                            {{ str_replace('_', ' ',$data['role_code']) }}
                          </td>
                            <td>
                            {{ $data['users_app_text'] }}
                          </td>
                          ...
                          <td>
                            ...
                            <?php }else if($data['status'] == 2){ //delete ?>
                            
                            <?php } ?>
                          </td>
                          <td class="table-action">
                            <span>
                              <a href="{{ route('user.show',$data['id']) }}" title="View">
                            ...
                            <span>
                              <a href="{{ route('user.edit',$data['id']) }}" title="Edit">
                                ...
                                @if($data['role_code'] != 'SUPER_ADMIN')
                                <span>
                                  <a data-url="{{route('admin.user.delete',['admin_user_id'=>$data['id']])}}" type="button" title="Delete"
                  ...
                  <nav class="pagination-grid" aria-label="Page  navigation example">
                    <ul class="pagination">
                        <?php if($AdminUserData->hasPages()) { ?>
                      {!! $AdminUserData->appends(Request::except('page'))->render() !!}
                        <?php } ?>
                    </ul>
                  </nav>
                  <!-- /.card-body -->

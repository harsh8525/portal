...
class LogExport implements FromCollection, WithHeadings, WithStyles, WithCustomStartCell, WithEvents, WithDrawings
{
    ...
    function __construct($filter, $transactionDate, $fromDate, $toDate)
    {
        ...
        $this->orderTotalAmount = CustomerActivityLog::getLogReport($this->filter, false, true);
    ...
    {
        $logData = CustomerActivityLog::getLogReport($this->filter, false);

        $data = [];
        foreach ($logData as $key => $value) {
            $code =$value['getRole']->name??'';
            if($value->customer_id != 'guest'){

                $customer = Customer::where('id',$value['customer_id'])->value('first_name');
                $customerName = $customer ? $customer : $value->customer_id;
            ...
            $temp = [
                'sr.no' => ++$key,
                'customer_id' => ucwords($customerName),
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A2', "LOG REPORT");
